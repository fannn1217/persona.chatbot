...
tf.flags.DEFINE_integer("max_sentence_len", 100, "Maximum Sentence Length")

tf.flags.DEFINE_string(
  "input_dir", os.path.abspath("./data/persona"),
  "Input directory containing original CSV data files (default = './data')")

tf.flags.DEFINE_string(
  "output_dir", os.path.abspath("./data/persona"),
  "Output directory for TFrEcord files (default = './data')")

FLAGS = tf.flags.FLAGS

TRAIN_PATH = os.path.join(FLAGS.input_dir, "train_shuf.csv")
...
def create_example_train(row, vocab):
  ...
  """
  context, utterance, persona, label = row
  context_transformed = transform_sentence(context, vocab)
  utterance_transformed = transform_sentence(utterance, vocab)
  persona_transformed = transform_sentence(persona, vocab)
  context_len = len(next(vocab._tokenizer([context])))
  utterance_len = len(next(vocab._tokenizer([utterance])))
  persona_len = len(next(vocab._tokenizer([persona])))
  ...
  example.features.feature["persona"].int64_list.value.extend(persona_transformed)
  example.features.feature["context_len"].int64_list.value.extend([context_len])
  example.features.feature["utterance_len"].int64_list.value.extend([utterance_len])
  example.features.feature["persona_len"].int64_list.value.extend([persona_len])
...
def create_example_test(row, vocab):
  ...
  Returnsthe a tensorflow.Example Protocol Buffer object.
  """
  context, utterance, persona = row[:3]
  distractors = row[3:]
  context_len = len(next(vocab._tokenizer([context])))
  utterance_len = len(next(vocab._tokenizer([utterance])))
  persona_len = len(next(vocab._tokenizer([persona])))
...
if __name__ == "__main__":
  ...
  input_iter = (x[0] + " " + x[1] + " " + x[2]for x in input_iter) #context+utter+persona
